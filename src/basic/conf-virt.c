...
{
  log_debug("CPUID func %x %x\n", *eax, *ecx);
  __cpuid(*eax, *eax, *ebx, *ecx, *edx);
  log_debug("CPUID result %x %x %x %x\n", *eax, *ebx, *ecx, *edx);
}
...
static ConfidentialVirtualization detect_tdx(void)
{
  ...
  cpuid(&eax, &ebx, &ecx, &edx);
  log_debug("CPUID max function: %x %x %x %x\n", eax, ebx, ecx,edx);
...
detect_hypervisor(void)
{
  ...
  log_debug("CPUID is hypervisor: %s\n", is_hv ? "yes" : "no");
  return is_hv;
}
